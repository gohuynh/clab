function samp_and_save(psd_info)
%% Global variables
...
for k = activeChans
    addAnalogInputChannel(s, 'Dev4', ['ai' num2str(k)], 'Voltage');
end

% Add Clock and Trigger
addTriggerConnection(s, 'external', 'Dev4/PFI1', 'StartTrigger');
cc = addClockConnection(s, 'external', 'Dev4/PFI7', 'ScanClock');

% Session Settings
s.IsContinuous = true;
s.Rate = sampR;
lh = addlistener(s,'DataAvailable', @save_data);
...
if myInfo.plotLive
    ...
    aline1 = animatedline(fig_one,'Color','blue','MaximumNumPoints',sampR);
    
    title(fig_two,['Measured Voltage of ' fig_two_title])
    ylabel(fig_two,'Voltage (V)')
    ylim(fig_two,[myInfo.ylow myInfo.yhigh]);
    aline2 = animatedline(fig_two,'Color','red','MaximumNumPoints',sampR);
end
